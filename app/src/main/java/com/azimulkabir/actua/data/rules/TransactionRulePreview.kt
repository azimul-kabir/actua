...
internal object TransactionRulePreview {
    fun map(draft: Transaction, result: RuleRunResult, choices: RulePreviewChoices): Transaction {
        ...
        val payee = when {
            draft.type == Type.TRANSFER -> draft.payee
        ...
        val type = when {
            draft.type == Type.TRANSFER -> Type.TRANSFER
            "amount" !in changed || preview.amountCents == 0L -> draft.type
            preview.amountCents > 0 -> Type.INCOME
            else -> Type.EXPENSE
        }
        return draft.copy(
            account = account,
