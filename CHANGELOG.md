...
- Added app-wide favorites for categories, accounts and report dashboards: star a category, account or report from its own screen and it shows up in Home's Favorite Categories/Accounts sections and in a new **Favorites** filter on Budget (composes with Budget's existing filters instead of replacing them) and continues to power the existing Favorites widget. Favorites are a device-local preference, not synced budget data, and hidden/deleted/closed entries are filtered out automatically everywhere they'd otherwise appear
